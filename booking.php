...
$rates = ["simple" => 50, "deluxe" => 100, "suite" => 150];
$base_price = isset($rates[$the_room_category]) ? $rates[$the_room_category] : 5000;
$days = ceil((strtotime($check_out) - strtotime($check_in)) / 86400);
$total_amt = $base_price * ($days > 0 ? $days : 1);
...
if(isset($_POST['add_booking'])){
    ...
    $check_cust = "SELECT cust_id FROM customer WHERE passport_no = '$passport_no' OR cust_email = '$cust_email' LIMIT 1";
    ...
    if(!$warning) {
        ...
        if(!$cust_exists) {
            $query = "INSERT INTO customer(cust_email, cust_phone, passport_no, country, dob, f_name, l_name) 
                      VALUES('$cust_email', '$cust_phone', '$passport_no', '$country', '$dob', '$f_name', '$l_name')";
        ...
        if(!empty($_POST['dep_name'])) {
            ...
            $query = "INSERT INTO dependents(cust_id, dep_name, passport_no) VALUES('$the_cust_id', '$dep_name', '$dep_pass')";
        ...
        mysqli_query($con, "INSERT INTO booking(cust_id, Booking_id, room_id, branch_id) VALUES('$the_cust_id', '$booking_id', '$room_id', '$branch_id')");
        mysqli_query($con, "INSERT INTO bill(cust_id, Bill_id, Amount, Payment_type) VALUES('$the_cust_id', '$bill_id', '$total_amt', '$payment_type')");
        mysqli_query($con, "INSERT INTO forr(Booking_id, room_id, check_in_date, check_out_date) VALUES('$booking_id', '$room_id', '$check_in', '$check_out')");
        mysqli_query($con, "INSERT INTO generates(Booking_id, Bill_id, booking_date) VALUES('$booking_id', '$bill_id', '$booking_date')");
...
<head>
    ...
    <style>
        .section { min-height: 100vh; background: url('./images/background2.jpg') no-repeat center center fixed; background-size: cover; padding: 50px 0; }
        .booking-form { background: rgba(0, 0, 0, 0.8); padding: 40px; border-radius: 15px; color: white; position: relative; }
        .form-control { background: rgba(255,255,255,0.1); border: 1px solid #444; color: #fff; height: 50px; margin-bottom: 15px; border-radius: 25px; }
        .form-control:focus { background: rgba(255,255,255,0.2); color: #fff; box-shadow: 0 0 10px #ff8846; }
        .submit-btn { background: #e35e0a; color: white; border: none; padding: 15px; width: 100%; border-radius: 25px; font-weight: bold; text-transform: uppercase; }
        #depText { color: #ff8846; cursor: pointer; font-weight: bold; margin-bottom: 20px; display: inline-block; }
        .error-msg { color: #ff4d4d; text-align: center; margin-bottom: 15px; font-weight: bold; }
        #removeBtn { background: #ff4d4d; border: none; color: white; border-radius: 50%; width: 30px; height: 30px; margin-left: 10px; cursor: pointer; }
    </style>
</head>
<body>
                ...
                <div class="col-md-10">
                    <div class="booking-form">
                        <div class="text-center mb-4">
                            ...
                            <p>Category: <?php echo ucfirst($the_room_category); ?> | Branch: <?php echo $branch_id; ?></p>
                        </div>

                        <?php if($warning): ?>
                            <div class="error-msg"><?php echo $warning; ?></div>
                        <?php endif; ?>
                        ...
                        <form action="" method="post" id="confirm_form">
                            ...
                            <h3 class="text-center mt-4">Total: RM <?php echo number_format($total_amt, 2); ?></h3>
                            <div class="text-center small text-muted" id="countdown">Session expires in 05:00</div>
                        </form>
                    </div>
                </div>
            </div>
